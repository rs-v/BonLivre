...
- Use `curl` against `http://127.0.0.1:5000` to drive protected API routes. `/health` is public; API routes need `Authorization: Bearer <password>` or `?password=` where the browser cannot set headers.
